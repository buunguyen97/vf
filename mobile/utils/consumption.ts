...
export function getEnergyPer1PercentWh(vehicle: any) {
  const batteryCapacityKwh = Number(vehicle?.battery_capacity_kwh) || 60;
  return batteryCapacityKwh * 1000 * 0.01;
}

export function getVehicleBaseConsumption(vehicle: any) {
  return Number(vehicle?.base_consumption_wh_km) || 150;
}

export function getDefaultKmPer1Percent(vehicle: any) {
...
export function getAdjustedDefaultConsumption(vehicle: any) {
  const energyPer1Percent = getEnergyPer1PercentWh(vehicle);
